...
class RegisterForm(FlaskForm):
    ...
    L_name = StringField(label='Last Name', validators=[Length(min=2, max=30), DataRequired()])
    User_name = StringField(label='User Name:', validators=[Length(min=2, max=30), DataRequired()])
    email_address = StringField(label='Email Address:', validators=[Email(), DataRequired()])
    phone = StringField(label='Phone Number:', validators=[Length(min=2, max=30), DataRequired()])
